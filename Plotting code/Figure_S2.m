% Produce the panels in Figure S2 of our manuscript. This script requires
...
figure(1); hold on;
p1 = plot(d_g,prob_mat_Alpha_plot,'r','linewidth',0.1);
p2 = plot(d_g,overall_probs_Alpha,'k','linewidth',3);
xlim([0,15])
ylim([0,0.35])
yticks(0:0.05:0.35)
xlabel({'Household generation time (days)'})
ylabel('Probability')
l = legend([p1(1),p2],{'Posterior samples','Overall distribution'});
l.FontSize = 15;
...
figure(2); hold on;
p1 = plot(d_g,prob_mat_Delta_plot,'r','linewidth',0.1);
p2 = plot(d_g,overall_probs_Delta,'k','linewidth',3);
xlim([0,15])
ylim([0,0.35])
yticks(0:0.05:0.35)
xlabel({'Household generation time (days)'})
...
p2 = plot(t_g,f_gD,'color',c2,'linewidth',3);
xlim([0,15])
xlabel({'Household generation time (days)'})
...
figure(1); export_fig Figures/Figure_S2/A.pdf -nocrop -painters -transparent
figure(2); export_fig Figures/Figure_S2/B.pdf -nocrop -painters -transparent
figure(3); export_fig Figures/Figure_S2/C.pdf -nocrop -painters -transparent
